...
- **逐帧演变** — 勾选后，噪点与全部 6 个故障滤镜的图案**随帧号变化**（确定性生成，重跑动画逐帧一致），VHS 跟踪条随帧滚动，配合"全部帧应用"实现动态故障/胶片颗粒动画；扫描线还可单独开启"隔行闪烁"
- **进度条与任务保护** — 大任务自动显示进度条（可取消）；任务过重时主动终止并提示，处理失败自动回滚，绝不卡死或崩溃
...
**做动画**：同时勾选 **"逐帧演变"**，噪点和故障滤镜的图案会随帧号变化——行撕裂/数据损坏/像素排序/置换扭曲/镜像撕裂每帧换一套图案，VHS 跟踪条位置逐帧滚动，噪点变成胶片颗粒动画。所有演变都是确定性的（同参数重跑动画逐帧一致），对话框预览直接显示**当前帧**的效果。扫描线的 **"隔行闪烁"** 可单独开启，产生经典 CRT 逐行闪动的效果。

> 提示：如果希望某几个滤镜保持固定图案、只有其他滤镜演变，把那些滤镜关掉或使用"固定噪点"即可。

**大任务保护**：应用前插件会估算工作量——预计耗时很长的任务会弹出提示并**主动终止**（建议缩小画布、减少帧数或滤镜数量）；常规大任务会显示**进度条**（可点取消，取消后自动回滚，不会留下半成品）；任何处理失败都会自动回滚并显示错误信息，不会让 Aseprite 崩溃。
